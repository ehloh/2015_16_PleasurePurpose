...
x.latinhib =       -1./(1+exp(-xx(1))); 
x.threshold =        100./(1+exp(-xx(2))); 
x.attribeta =        10./(1+exp(-xx(3))); 
...
for sm= 1: fp.nsamples
    ...
    %     d(tr, [col.pl1 col.pp1 col.pl2 col.pp2]) 
        ...
        for tb=2:ntb
            ...
            % Attribute-weighting 
            weightPL = 1/( 1+ exp(      tb*x.attribeta* ( abs(tr_fev(tb, 1)-tr_fev(tb, 3)) - abs(tr_fev(tb, 2)-tr_fev(tb, 4)) )     ) ); 
